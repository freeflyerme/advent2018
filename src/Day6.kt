...
fun part6_2() {
    val MAX_DIST = 10000  // reset to 10000 for input
    val INVALID_DIST = MAX_DIST * 2
    val buffer = 25  // number of points i 50, so we don't need to look past 25 in each direction
    ...
    val xLen = maxX - minX + 1 + buffer
    val yLen = maxY - minY + 1 + buffer
    ...
    for (x in 0 until xLen ) {
        for (y in 0 until yLen) {
            ...
            for ((pointName, p) in stdPoints.withIndex()) { // Lesson: withIndex to simplify things
                // calculate distance to p, add it
                val dist = manhattanDist(currentPoint, p)
                if (grid[x][y] == INVALID_DIST) {
                    grid[x][y] = dist
                } else {
                    grid[x][y] = grid[x][y] + dist
                }
            }
        }
    }
...
//    var minGrid = List(xLen) { MutableList(yLen) { 0 } }
    var sum = 0
    for (x in 0 until xLen) {
        for (y in 0 until yLen) {
            if (grid[x][y] < MAX_DIST) {
                sum++
//                minGrid[x][y] = 1
            }
        }
